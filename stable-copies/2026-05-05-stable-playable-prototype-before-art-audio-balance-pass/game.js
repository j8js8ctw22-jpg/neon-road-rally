...
class Renderer {
  ...
  getPlayerHitbox() {
    const run = this.game.run;
    const scale = run.airborne ? 1.06 : 1;
    const x = this.laneCenter(run.renderLaneFloat);
    const y = this.getPlayerScreenY() - 2 * scale;
    return rectFromCenter(x, y, 76 * scale * 0.86, 118 * scale * 0.8);
  }
  ...
  drawDebug() {
    ...
    ctx.fillRect(x, y, 304, lines.length * 19 + 14);
    ctx.strokeStyle = "#ffe45e";
    ctx.strokeRect(x, y, 304, lines.length * 19 + 14);
...
function drawPlayerCar(ctx, x, y, carConfig, state) {
  ...
  const scale = state.airborne ? 1.06 : 1;
  const w = 76 * scale;
  const h = 118 * scale;
  ctx.save();
  ctx.translate(x, y);
  if (state.crashFlash > 0) {
    ctx.translate((Math.random() - 0.5) * 10 * state.crashFlash, (Math.random() - 0.5) * 8 * state.crashFlash);
  }
  ...
  if (state.boosting) {
    ...
    ctx.lineTo(-w * 0.06, h * 0.82 + Math.random() * 18);
    ...
    ctx.lineTo(w * 0.26, h * 0.75 + Math.random() * 15);
  ...
  ctx.globalAlpha = state.airborne ? 0.36 : 0.55;
  ctx.fillStyle = "#000";
  ctx.beginPath();
  ctx.ellipse(0, h * 0.43, w * 0.48, h * 0.14, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.globalAlpha = 1;

  ctx.fillStyle = "#05070f";
  ctx.fillRect(-w * 0.48, -h * 0.22, w * 0.14, h * 0.72);
  ctx.fillRect(w * 0.34, -h * 0.22, w * 0.14, h * 0.72);
  ...
  ctx.fillStyle = glass;
  ctx.fillRect(-w * 0.24, -h * 0.28, w * 0.48, h * 0.2);
  ctx.fillStyle = shade(glass, -42);
  ctx.fillRect(-w * 0.18, -h * 0.03, w * 0.36, h * 0.16);

  ctx.fillStyle = stripe;
  ctx.shadowBlur = 10;
  ctx.shadowColor = stripe;
  ctx.fillRect(-w * 0.055, -h * 0.48, w * 0.11, h * 0.87);
  ctx.fillRect(-w * 0.33, h * 0.32, w * 0.66, h * 0.055);
  ctx.shadowBlur = 0;

  ctx.fillStyle = "#f8fbff";
  ctx.fillRect(-w * 0.34, -h * 0.47, w * 0.16, h * 0.055);
  ctx.fillRect(w * 0.18, -h * 0.47, w * 0.16, h * 0.055);
  ctx.fillStyle = "#ff273f";
  ctx.fillRect(-w * 0.34, h * 0.44, w * 0.16, h * 0.045);
  ctx.fillRect(w * 0.18, h * 0.44, w * 0.16, h * 0.045);

  ctx.fillStyle = shade(body, 28);
  ctx.fillRect(-w * 0.38, -h * 0.55, w * 0.76, h * 0.06);
  ctx.fillStyle = contrastText(body);
  ctx.font = `${Math.max(8, 10 * scale)}px Trebuchet MS, Verdana, sans-serif`;
  ctx.textAlign = "center";
  ctx.fillText(String(carConfig.name || "CAR").slice(0, 10).toUpperCase(), 0, h * 0.08);
  ctx.restore();
}
...
class NeonRoadRally {
  ...
  showCustomizeScreen(message = "") {
    ...
    this.layer.innerHTML = `
      <section class="panel split">
        <div class="form-stack">
          <h2>Customize Car</h2>
          <p class="hint">These colors are used directly in the race.</p>
          <div class="field">
    ...
    ["carName", "bodyColor", "stripeColor", "windowColor"].forEach((id) => {
  ...
  renderCarPreview() {
    ...
    drawPlayerCar(ctx, canvas.width / 2, canvas.height / 2 + 16, this.readCarForm(), {
      boosting: true,
      airborne: false,
      laneChanging: false,
      crashFlash: 0
