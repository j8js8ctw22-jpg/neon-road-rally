...
vm.runInContext(`
  ...
  const quickDashDistance = getDriftDashLaneDistanceForHold(0.12);
  const shortDashDistance = getDriftDashLaneDistanceForHold(0.24);
  const longDashDistance = getDriftDashLaneDistanceForHold(DRIFT_TUNING.maxChargeSeconds);
  const cappedDashDistance = getDriftDashLaneDistanceForHold(5);
  const shortBoost = getDriftDashBoostForRelease(0.16, quickDashDistance);
  const longBoost = getDriftDashBoostForRelease(DRIFT_TUNING.maxChargeSeconds, longDashDistance);
  const cappedBoost = getDriftDashBoostForRelease(5, LANES);
  const driftElapsedForLaneCut = (lanes) => Math.max(0, lanes / DRIFT_TUNING.dashLanesPerSecond - DRIFT_TUNING.initialDashSeconds);
  const threeTapLaneTime = 3 * INPUT_CONFIG.laneChangeDurationSeconds;
  assert(Math.abs(DRIFT_TUNING.maxChargeSeconds - 0.55) <= 0.04, "Max drift dash hold should cap around 0.55 seconds");
  assert(DRIFT_TUNING.releaseCooldownSeconds <= 0.22, "Drift dash cooldown should stay short for responsive repeated inputs");
  assert(DRIFT_TUNING.maxBoostMultiplier < SPEED_TUNING.manualBoostMultiplier, "Space boost should remain stronger than drift dash release boost");
  assert(DRIFT_TUNING.maxBoostMultiplier < SPEED_TUNING.padBoostMultiplier, "Boost pads should remain stronger than drift dash release boost");
  assert(driftElapsedForLaneCut(2) < 2 * INPUT_CONFIG.laneChangeDurationSeconds, "Drift dash should cross two lanes faster than repeated tap lane changes");
  assert(driftElapsedForLaneCut(3) < threeTapLaneTime, "Drift dash should beat three normal tap lane changes when used correctly");
  assert(quickDashDistance > 1 && quickDashDistance < 1.3, "Quick drift dash should cut about one lane");
  assert(shortDashDistance > 2 && shortDashDistance < 2.35, "Short committed drift dash should cross multiple lanes");
  ...
  assert(app.run.driftFullChargeReady === false, "Sub-cap dash should not hit the max-hold cue yet");
  app.updateDriftState(0.24);
  ...
  assert(app.run.maxDriftHold >= 0.54, "Max drift hold should reach the 0.55-second cap");
