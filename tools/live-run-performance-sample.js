...
async function runRoute(page, route) {
  ...
  const result = await page.evaluate((routeConfig) => {
    ...
    const result = {
      ...
      performanceEffectScale: Number((run.performanceEffectScale || 1).toFixed(2)),
      renderEffectScaleMin: Number((run.renderEffectScaleMin || 1).toFixed(2)),
      neonFlowTotalEarned: run.neonFlowTotalEarned || 0,
      flowBreaksArmed: run.flowBreaksArmed || 0,
      flowBreakArmed: Boolean(run.flowBreakArmed),
      flowBreaksTriggered: run.flowBreaksTriggered || 0,
      flowBreakHazardsCleared: run.flowBreakHazardsCleared || 0,
      ...
      flowBreakMinorHazardsCleared: run.flowBreakMinorHazardsCleared || 0,
      flowBreakMajorHazardsCleared: run.flowBreakMajorHazardsCleared || 0,
      flowBreakSparksCreated: run.flowBreakSparksCreated || 0,
      flowBreakSparksCollected: run.flowBreakSparksCollected || 0,
